class CnbGrid extends Grid{
  ...
  getNeighbors(x, y, opponent){
    let neighbor = [];
    //LEFT
    if (x-1 > 0 && (this.map[x-1][y] === 0 || this.map[x-1][y] === 6 || this.map[x-1][y] === 2 || this.map[x-1][y] === 7) ){
      let overlap = false;
    ...
    //RIGHT
    if (x+1 < this.map[0].length && (this.map[x+1][y] === 0 || this.map[x+1][y] === 6 || this.map[x+1][y] === 4 || this.map[x+1][y] === 7)) {
      let overlap = false;
      for (let op of opponent) {
        if (op.gridX === x + 1 && op.gridY === y)
          overlap = true;
    ...
    //UP
    if (y-1 > 0 && (this.map[x][y-1] === 0 || this.map[x][y-1] === 6 || this.map[x][y-1] === 3 || this.map[x][y-1] === 7)){
      let overlap = false;
      for (let op of opponent) {
    ...
    //DOWN
    if (y+1 < this.map.length && (this.map[x][y+1] === 0 || this.map[x][y+1] === 6 || this.map[x][y+1] === 5 || this.map[x][y+1] === 7)){
      let overlap = false;
      for (let op of opponent) {
        if (op.gridX === x && op.gridY === y + 1)
          overlap = true;
